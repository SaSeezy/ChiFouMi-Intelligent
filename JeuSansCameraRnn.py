...
# Embedded file name: /home/aSara/Documents/TRAVAIL/ING2/PROJET/Travail/ELABORATION1/125061_FILES/REPTILE/TRAVAIL/IHM_sara/FINAL_SASSOU/test_phpadmin/JeuSansCameraRnn.py
...
class JeuSansCameraRnn(InterfaceJeu):

    def __init__(self, session):
        self.sessionLogin = session
        self.num = 0
        self.idPartie = insererPartie(self.sessionLogin, 2)
    ...
    def setTitleJeu(self):
        ...
        fichier.close()
        if lang == 'fr':
            self.setTitle('Jeu Chifoumi: Réseau de neurones')
        else:
            self.setTitle("Chifoumi's Game: Neural network")
    ...
    def finpartie(self):
        ...
        dt = getdata()
        updatePartie(self.idPartie, dt[2], dt[1], dt[3])
        renitialiser()
        self.destroy()
    ...
    def traduction_page(self):
        self.destroy()
        self.choix_lang()
        x = JeuSansCameraRnn(self.sessionLogin)
    ...
    def pierre(self,event=""):
        traduction = {"r":"0","p":"1","s":"2" }
        trad={"0":"p","1":"f","2":"c"}
        dt = rnn_prediction("r")
        self.num+=1
        dt[0]=traduction[dt[0]]
        dt[1]=traduction[dt[1]]
        insererCoup(self.num, self.idPartie, 'p', trad[dt[1]])
        self.updateIHM(dt)
    ...
    def feuille(self,event=""):
        ...
        dt[0]=traduction[dt[0]]
        dt[1]=traduction[dt[1]]
        insererCoup(self.num, self.idPartie, 'f', trad[dt[1]])
        self.updateIHM(dt)
    ...
    def ciseau(self,event=""):
        ...
        dt[0]=traduction[dt[0]]
        dt[1]=traduction[dt[1]]
        insererCoup(self.num, self.idPartie, 'c', trad[dt[1]])
        self.updateIHM(dt)

if __name__ == "__main__":
    x = JeuSansCameraRnn("badi")